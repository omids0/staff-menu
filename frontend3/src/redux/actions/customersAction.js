...
export const addNewCustomerAction = (customer) => async (dispatch) => {
  ...
  try {
    const response = await axios.post("/api/customers/addnewcustomer", {
      customer,
    });
...
export const findCustomerByIdAction = (customerId) => async (dispatch) => {
  ...
  try {
    const response = await axios.post("/api/customers/findbycustomerid", {
      customerId,
    });
...
export const findCustomerAction = (id) => async (dispatch) => {
  dispatch({ type: "FIND_CUSTOMER_REQUEST" });
  try {
    const response = await axios.post("/api/customers/findcustomer", {
      id,
    });
    dispatch({ type: "FIND_CUSTOMER_SUCCESS", payload: response.data });
  } catch (error) {
    dispatch({ type: "FIND_CUSTOMER_FAILED", payload: error.message });
  }
};
...
export const editCustomerAction = (customer) => async (dispatch) => {
  ...
  try {
    await axios.post("/api/customers/editcustomer", {
      customer,
    });
